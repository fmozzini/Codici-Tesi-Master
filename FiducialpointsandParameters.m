...
% Parameters from SCG
    ...
    PEP = zeros(length(qrs_I)-finepicchi,2); % Q-AO
...
%         tutti i parameters per questa finestra sono NaN
        ...
        Q(i,1:2) = NaN; fine_T(i,1:2) = NaN; T(i,1:2) = NaN; RR(i,1:2) = NaN; R_MC1(i,1) = NaN; R_AC1(i,1) = NaN; R_AO(i,1) = NaN; PEP(i,1) = NaN; 
...
%             qrs_I_sec = (qrs_I(i)./fs_ECG);
%             ricercariga = round(round(qrs_I_min)/5);
%             bpm_min = HR_5min(ricercariga+1);
%             ricercariga = round(round(qrs_I_sec)/30);
            ricercariga = ceil(i/30);
            bpm_30heartbeat = HR_30heartbeat(ricercariga);

            if sex{m} == 'F'
               QS2maxline = -0.0020*bpm_30heartbeat+0.549;
           else
               QS2maxline = -0.0021*bpm_30heartbeat+0.546;
            end 
...
%                 FINESTREBATTITO_SCG(count,1:length(finestrabattito_SCG)) = finestrabattito_SCG;
               ...
               locsmaxdopofineT = find(locs > fine_T_SCG-0.004*fs_SCG); % meno 0.4 ms prima della fine dell'onda T
...
%             a = subplot(211)
%             plot((qrs_I(i)-window_ECG:qrs_I(i+1)-window_ECG)./1024,finestrabattito_ECG),xlabel('[s]'),ylabel('[mV]'); hold on; plot(qrs_I(i)./1024,qrs_AMP(i),'*r'); hold on; 
%             plot(T(i,1)/1024,T(i,2),'*g'); hold on;
%             xline(qrs_I(i)/1024); hold on; 
% %             xline(T(i,1)/1024,'--g'); hold on;
...
%             text(fine_T(i,1)/1024,fine_T(i,2),'         T-end')
%             text(Q(i,1)/1024,Q(i,2),' Q')
%             title('Ecg')
%             b = subplot(212)
%             plot((qrs1-window_SCG:qrs2-window_SCG)./64,finestrabattito_SCG),xlabel('[s]'),ylabel('[millig]'); hold on; 
%             for r = 1:length(row)
...
% %             plot((qrs1-window_SCG+maxsort(2,2)-1)/64,maxsort(2,1),'*r')
% %             plot((qrs1-window_SCG+maxsort(3,2)-1)/64,maxsort(3,1),'*r')
%             title('Scg')
% %             sgtitle(i)
% %             R_MC1(i,1)
%              R(i,3)
...
% % % %     
    %% Estraggo i parameters 
            [tIVCAO,tIVCAC,ampIVCAO,ampIVCAC,slopeIVCAO,lvet,qs2,qt,qtc,tIVCMC,tIVCRE,tIVCminAORE,tIVCminAC,ampIVCMC,ampIVCRE,ampIVCminAORE,...
                ampIVCminAC,slopeminAORERE,slopeminACAC,RdivT,pep] = extractfeatures(64,1024,AO(i,1),AO(i,2),IVC(i,1),IVC(i,2),AC(i,1),AC(i,2),...
            ...
            LVET(i,1) = lvet; QS2(i,1) = qs2; QT(i,1) = qt; QTc(i,1) = qtc; PEP(i,1) = pep;
            if isnan(slopeIVCAO)
                slope_IVCAO(i,1) = 0;
            else
...
%             if R_MC1(i,1) < 0 % provo a mettere che può essere positivo ma non più di 5 ms dopo R 
             if R_MC1(i,1) < -0.004*fs_SCG
                ...
                amp_IVCAO(i,1) = NaN; amp_IVCAC(i,1) = NaN; amp_IVCMC(i,1) = NaN; amp_IVCRE(i,1) = NaN; amp_IVCminAORE(i,1) = NaN; amp_IVCminAC(i,1) = NaN; 
                slope_IVCAO(i,1) = NaN; slope_minAORERE(i,1) = NaN; slope_minACAC(i,1) = NaN; LVET(i,1) = NaN; QS2(i,1) = NaN; QT(i,1) = NaN; QTc(i,1) = NaN;
                LVET(i,1) = NaN; R_div_T(i,1) = NaN; R_AO(i,1) = NaN; PEP(i,1) = NaN;
...
%                     R_div_T(i,1) = NaN; R_AO(i,1) = NaN; PEP(i,1) = NaN;
                ...
                amp_IVCAO(i,1) = NaN; amp_IVCAC(i,1) = NaN; amp_IVCMC(i,1) = NaN; amp_IVCRE(i,1) = NaN; amp_IVCminAORE(i,1) = NaN; amp_IVCminAC(i,1) = NaN; 
                slope_IVCAO(i,1) = NaN; slope_minAORERE(i,1) = NaN; slope_minACAC(i,1) = NaN; LVET(i,1) = NaN; QS2(i,1) = NaN; QT(i,1) = NaN; QTc(i,1) = NaN;
                R_MC1(i,1) = NaN; R_AC1(i,1) = NaN; R_div_T(i,1) = NaN; R_AO(i,1) = NaN; PEP(i,1) = NaN;
...
%     FINESTREBATTITO_ECG(count+1:end,:) = []; FINESTREBATTITO_SCG(count+1:end,:) = []; picchi_totali(1:iniziopicchi-1,:) = [];
    ...
    R_AO(1:iniziopicchi-1,:) = []; PEP(1:iniziopicchi-1,:) = [];
    R_AC1 = R_AC1./64;
    R_MC1 = R_MC1./64;
    R_AO1 = R_AO./64;
...
%      Salvo i dati SU FINESTRA DI 10 SEC
        ...
        't_IVCminAC','amp_IVCAO','amp_IVCAC','amp_IVCMC','amp_IVCRE','amp_IVCminAORE','amp_IVCminAC','R_div_T','slope_IVCAO','slope_minAORERE',...
        'slope_minACAC','LVET','QS2','QT','QTc','RR','GN','R_AO','R_MC1','PEP','R_AC1')
