...
% Parameters from SCG
    ...
    tempofinoasonno = (tempofinoasonno_sec/60)*fs_ECG;
    duratasonnodainizio = (duratasonnodainizio_sec/60)*fs_ECG;
...
%             R_AC1(i,1) = AC(i,1)-R_SCG; % se positivo AC è dopo R, se negativo AC è prima di AC
            Q_AC1(i,1) = AC(i,1)-Q_SCG;
    
            if R_MC1(i,1) < 0 % non sono in grado di trovare la sistole --> devo settare qualcosa a zero? Allora non trovo nemmeno la sistole
...
%                 Q(i,1:2) = NaN; fine_T(i,1:2) = NaN; T(i,1:2) = NaN; RR(i,1:2) = NaN;
                ...
                amp_IVCAO(i,1) = NaN; amp_IVCAC(i,1) = NaN; amp_IVCMC(i,1) = NaN; amp_IVCRE(i,1) = NaN; amp_IVCminAORE(i,1) = NaN; amp_IVCminAC(i,1) = NaN; 
                slope_IVCAO(i,1) = NaN; slope_minAORERE(i,1) = NaN; slope_minACAC(i,1) = NaN; LVET(i,1) = NaN; QS2(i,1) = NaN; QT(i,1) = NaN; QTc(i,1) = NaN;
%                 disp('MC troppo avanti')
%                 pause
            else % MC va bene ma non va bene AC
%                 if AC(i,1) > QS2maxline
                  if (Q_AC1(i,1)/64) > 0.44
                    tag5 = tag5+1;
                    R(i,3) = 5;
    %                 MC(i,1:2) = NaN; RE(i,1:2) = NaN; AO(i,1:2) = NaN; IVC(i,1:2) = NaN; minAO_RE(i,1:2) = NaN;
    %                 t_IVCAO(i,1) = NaN; t_IVCAC(i,1) = NaN; t_IVCMC(i,1) = NaN; t_IVCRE(i,1) = NaN; t_IVCminAORE(i,1) = NaN; t_IVCminAC(i,1) = NaN; 
                    amp_IVCAO(i,1) = NaN; amp_IVCAC(i,1) = NaN; amp_IVCMC(i,1) = NaN; amp_IVCRE(i,1) = NaN; amp_IVCminAORE(i,1) = NaN; amp_IVCminAC(i,1) = NaN;
                    slope_IVCAO(i,1) = NaN;slope_minAORERE(i,1) = NaN;
                    LVET(i,1) = NaN; QS2(i,1) = NaN; R_MC1(i,1) = NaN;
%                     disp('AC oltre linea max')
%                     pause
                 end 
            end 
% 
...
%              close all
    ...
    for i = 1:length(R)
         ...
         LVET(i,2) = tag; QS2(i,2) = tag; QT(i,2) = tag; QTc(i,2) = tag; R_MC1(i,2) = tag; R_AC1(i,2) = tag;
    end 
        
    tag0 = length(R)-tag1-tag2-tag3-tag4-tag5;
...
%     Salvo i dati (fiducial points e parameters)
    ...
    'R','tag0','tag1','tag2','tag3','tag4','tag5','Perc_analizzati')
