...
% Fiducial Points
    ...
    LVET = zeros(length(qrs_I)-finepicchi,3); % AC-AO
    QS2 = zeros(length(qrs_I)-finepicchi,3); % P-AC
    QT = zeros(length(qrs_I)-finepicchi,3); % Q-fine onda T
    QTc = zeros(length(qrs_I)-finepicchi,3); % Q-fine onda T (corretto) 
% Parameters from SCG
    t_IVCAO = zeros(length(qrs_I)-finepicchi,2);
    t_IVCAC = zeros(length(qrs_I)-finepicchi,2);
    amp_IVCAO = zeros(length(qrs_I)-finepicchi,2);
    amp_IVCAC = zeros(length(qrs_I)-finepicchi,2);
    slope_IVCAO = zeros(length(qrs_I)-finepicchi,2);
...
%             if length(finestrabattito_ECG)>400
%                 lunghezza_giusta = lunghezza_giusta + 1;
            ...
            %% Cerco il punto Q e poi di conseguenza 
                ...
                Q(i,:) = [qrs_I(i)-window_ECG+locspicchiECG(locsprimaR_ECG(end))-1 pksprimaR_ECG(end)]; %così è pos in sec in ECG
                Q_SCG_locs = (locspicchiECG(locsprimaR_ECG(end))/1024)*64;
                QS2maxline = Q_SCG_locs + 0.39*64; % QUESTA VALE ANCHE PER L'ONDA T !!
    ...
    %            QS2maxline_ECG = Q_ECG_locs + 0.44*1024; 
    ...
    %            pksdopoR_ECG = picchiECG(locsdopoR_ECG)
               ...
               numero = 0;
               for p = 1:length(pksdopoRprimaQT_ECG)
                   if (pksdopoRprimaQT_ECG(p) > 0)
               ...
               % proviamo -> se T è abbastanza avanti ed il segnale è corto,
               % devo ridurre la finestra di xr
...
%             end % chiude il check sulla lunghezza della finestra
        else 
            t_IVCAO(i,2) = 2;
            t_IVCAC(i,2) = 2;
            amp_IVCAO(i,2) = 2;
            ...
            QT(i,3) = 2;
            QTc(i,3) = 2;
            R(i,:) = [qrs_I(i) qrs_AMP(i) 2];

            % QUI FACCIO IL CONTROLLO E VEDO SE DEVO METTERE ALTRI TAG O
            % LASCIARE 0
            R_MC1 = R(i,1)-MC(:,1); % se positivo R è dopo MC, se negativo R è prima di MC --> la maggior parte delle volte R dovrebbe essere dopo MC, quindi +
            R_AC1 = AC(i,1)-R(:,1); % se positivo AC è dopo R, se negativo AC è prima di AC

            if R_MC1 <= 0 % non sono in grado di trovare la sistole --> devo settare qualcosa a zero?
                t_IVCAO(i,2) = 3;
                t_IVCAC(i,2) = 3;
                amp_IVCAO(i,2) = 3;
                amp_IVCAC(i,2) = 3;
...
%     Salvo i dati (fiducial points e parameters)
    ...
    save(['C:\Users\feder\Desktop\Tesi\Data\Parameters SCG\' 'Parameters SCG-' name],'t_IVCAO','t_IVCAC','amp_IVCAO','amp_IVCAC','slope_IVCAO','LVET','QS2','QC','QTc')
    save(['C:\Users\feder\Desktop\Tesi\Data\Fiducial Points SCG\' 'Fiducials SCG-' name],'AO','RE','IVC','AC','MC','R','minAO_RE','minbeforeAC','Q')
