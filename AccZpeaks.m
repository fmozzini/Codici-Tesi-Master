...
for i = N:N
    ...
    % Cerco la notte 
    Hour_add = 2*3600;
    Inizio = datevec(Inizio_Holter{i}); 
    Sonno = datevec(Periodo_Sonno{i}); 
    durata_sec = etime(Sonno,Inizio)
    durata_h = durata_sec/3600
    Night = durata_h + 2;
    Night_sec = round(Night*3600*64);
    template = Acc_filt.z_filt(Night_sec:Night_sec+10*fs_SCG);
    plot(Night_sec:Night_sec+10*fs_SCG,template),title('Template di 10 sec preso durante la notte')
    pause

    for i = 1:n_window-1 
    %for i = 1:2 
        [~, pos_picchi, amp_picchi,Template] = SCG_template_matching_corr(Acc_filt.z_filt((i-1)*wind_30sec+1:wind_30sec*i),fs_SCG,'x',template)
